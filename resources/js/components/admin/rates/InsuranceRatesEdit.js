...
export default class InsuranceRatesEdit extends Component
{
    constructor(props)
    {
        super(props)
    ...
    componentDidMount()
    {
        ax.get(`/api/admin/insurance-rates/${this.props.match.params.id}`, {
            headers: {
                'admin-token': token(),
            }
        }).then(response => {
    ...
    handleInputChange(e)
    {
        ...
        this.setState(prevState => {
            return {
                rate: { ...prevState.rate, rate: value }
            }
        })
    }
    ...
    handleEditRate(e)
    {
        ...
        ax.put(`/api/admin/insurance-rates/${this.state.rate.id}`, {
            rate: rate,
        }, {
            headers: {
                'admin-token': token(),
            ...
            n.done()
            this.props.history.push('/insurance-rates')
        }).catch(reason => {
        ...
        return (
            <div className="admin-marketer-panel-content">
                <div className="card shadow-sm p-3 p-md-4">
                    <h4 className="mb-4">ویرایش نرخ</h4>
                    {this.state.loading &&
                    ...
                    {!this.state.loading &&
                        <form onSubmit={this.handleEditRate} className="admin-marketer-panel-form">
                            ...
                            <div className="text-left text-sm-right">
                                ...
                                <Link to={'/insurance-rates'} className="btn btn-danger">انصراف</Link>
                            </div>
                        </form>
                    }
                </div>
